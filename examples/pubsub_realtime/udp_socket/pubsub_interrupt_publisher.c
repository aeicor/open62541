...
/* Note: Run thread based publisher or interrupt based publisher,
 * and subscriber.
 * Only one type of publisher can be started
 ...
 */
/* Loopback_T4 subscribes(T4) the counter data and
 *  loopback i.e publish the same data from Publisher(T5)
 * To run loop back in T4
 * uncomment: PUBLISHER, SUBSCRIBER, LOOPBACK_T4
 * comment: PUB_SYSTEM_INTERRUPT
 */
#define              LOOPBACK_T4

/* Loopback_T1 subscribes(T8) the published data from
 * T5 and store the counter data with timestamp in
 * the csv file
 * uncomment: PUBLISHER, SUBSCRIBER, LOOPBACK_T1
 * comment: PUB_SYSTEM_INTERRUPT
 */
//#define              LOOPBACK_T1
